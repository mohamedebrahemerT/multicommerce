...
<html>
  <head>
    ...
    <link rel="icon" href="{{url('/')}}/uploads/{{Auth::id()}}/favicon.ico" type="image/gif" sizes="16x16">
    <title>
        @php
        $shop = App\Useroption::where('user_id', Auth::id())->where('key', 'shop_name')->first();
        if($shop) {$shop_name=$shop->value;}
        @endphp
        @isset($shop_name){{$shop_name}}@endisset
    </title>
    ...
    <link rel="stylesheet" href="<?php echo asset('public/css/custom-'.'default.css') ?>" type="text/css" id="custom-style">
       <link href="https://fonts.googleapis.com/css2?family=Tajawal" rel="stylesheet">

        <style type="text/css">
          .size-box123 ul li.active {
...
.      .size-box123 ul li {
  ...
  <body onload="myFunction()"  style='font-family:  "Tajawal", "Segoe UI", arial;'     >
    <div id="loader"></div>
    <div class="pos-page"   @if(app()->getLocale() == 'ar') style="direction: rtl; "    @endif>
