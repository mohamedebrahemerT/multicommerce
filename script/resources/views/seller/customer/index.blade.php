			...
			<div class="float-left">
				<div class="input-group">
					<select class="form-control selectric" name="type">
						...
						<option value="active" class="text-danger">{{ __('active') }}</option>
						<option value="block" class="text-danger">{{ __('block') }}</option>
					...
					<tbody>
						...
						<tr id="row{{  $row->id }}">
							...
							<td><a href="{{ route('seller.customer.show',$row->id) }}">{{ $row->email }}</a></td>
								...
								<td><a href="{{ route('seller.customer.show',$row->id) }}">{{ $row->mobile }}</a></td>
								<td> 
									@if( $row->status == 1)
