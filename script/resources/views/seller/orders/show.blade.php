                                                                ...
                                                                <div class="card card-primary">
                                                                                        ...
                                                                                        <div class="col-5">
                                                                                            <a href="{{ url('/seller/product/'.$row->term->id.'/edit') }}">{{ $row->term->title ?? '' }}
                                                                                                @if($row->is_refundable)
                                                                                                    <span
                                                                                                        class="badge badge-danger mx-1">Refunded</span>@endif
                                                                                                <br>
                                                                                            </a>
                                                                                            @php
                                                                    ...
                                                                    <div class="card-footer">
                                                                        ...
                                                                        <div class="text-right">
                                                                            ...
                                                                            <form method="POST"
                                                                                  action="{{ route('seller.order.update',$info->id) }}"
                                                                                  accept-charset="UTF-8"
                                                                                  class="d-inline basicform">
                                                                                @csrf
                                                                                @method('PUT')

                                                                                <input type="checkbox" id="vehicle3"
                                                                                       name="mail_notify" value="1">
                                                                                <label
                                                                                    for="vehicle3"> {{ __('Notify To Customer') }}</label>
                                                                                ...
                                                                                <div class="btn-group">
                                                                                    <select class="form-control"
                                                                                            name="payment_status">
                                                                                        <option disabled="">
                                                                                            <b>{{ __('Select Payment Status') }}</b>
                                                                                        </option>
                                                                                        <option value="1"
                                                                                                @if($info->payment_status=='1') selected="" @endif>{{ __('Payment Complete') }}</option>
                                                                                        <option value="2"
                                                                                                @if($info->payment_status=='2') selected="" @endif>{{ __('Payment Pending') }}</option>
                                                                                        <option value="0"
                                                                                                @if($info->payment_status=='0') selected="" @endif>{{ __('Payment Cancel') }}</option>
                                                                                        <option value="3"
                                                                                                @if($info->payment_status=='3') selected="" @endif>{{ __('Payment Incomplete') }}</option>
                                                                                    </select>
                                                                                    &nbsp&nbsp
                                                                                    <select class="form-control"
                                                                                            name="status">
                                                                                        <option disabled="">
                                                                                            <b>{{ __('Select Order Status') }}</b>
                                                                                        </option>
                                                                                        <option value="pending"
                                                                                                @if($info->status=='pending') selected="" @endif>{{ __('Awaiting processing') }}</option>
                                                                                        <option value="processing"
                                                                                                @if($info->status=='processing') selected="" @endif>{{ __('Processing') }}</option>
                                                                                        <option value="ready-for-pickup"
                                                                                                @if($info->status=='ready-for-pickup') selected="" @endif>{{ __('Ready for pickup') }}</option>
                                                                                        <option value="completed"
                                                                                                @if($info->status=='completed') selected="" @endif>{{ __('Completed') }}</option>
                                                                                        <option value="archived"
                                                                                                @if($info->status=='archived') selected="" @endif>{{ __('Archived') }}</option>
                                                                                        <option value="canceled"
                                                                                                @if($info->status=='canceled') selected="" @endif>{{ __('Canceled') }}</option>
                                                        ...
                                                        <div class="col-12 col-lg-4">
                                                            <div class="card-grouping">
                                                                ...
                                                                <div class="card">
                                                                    ...
                                                                    <div class="card-body">
                                                                        <p class="mb-0">{{ $info->OrderNotes }}</p>
                                                                    </div>
                                                                </div>
@if($info->order_time != null)
                                                                <div class="card">
                                                                    <div class="card-header">
                                                                        <h4 class="card-header-title">{{ __('order_time_date') }}</h4>

                                                                    </div>
                                                                    <div class="card-body">
                                                                        <p class="mb-0">
                                 {{ __('order_time') }}:{{ $info->order_time }}
                                                                        </p>
                                                                        <p class="mb-0">
                          {{ __('order_date') }}:{{ $info->order_date }}
                                                </p>
                                                                ...
                                                                <div class="card">
                                                                    <div class="card-header">
                                                                        <h4 class="card-header-title">{{ __('emp') }}</h4>
                                                                    </div>
                                                                    <div class="card-body">
                                                                        @if($info->emp_id != null)
                                                                            <a href="{{url('/')}}/seller/users/{{ $info->emp->id }}/edit">{{ $info->emp->name }}
                                                                                (#{{ $info->emp->id }})</a>
                                                                        @else
                                                                            {{ __('Guest Customer') }}
                                                                        @endif
                                ...
                                @if($info->customer)
                            @foreach(App\Models\Useraddresses::
                            where('user_id',$info->customer->id )
                                            ...
                                            @endforeach
                                              ...
                                              @endif
                                                                        ...
                                                                        <p class="mb-0">{{ __('Shipping Method') }}
                                                                            : {{ $info->shipping_info->shipping_method->name ?? '' }}</p -->
                                                                    </div>
                                                                </div>
