                    ...
                    <div class="row">
                        <div class="col-sm-3">
                            <ul class="nav nav-pills flex-column">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link active" href="{{ url('seller/product/'.$info->id.'/image') }}"><i
                            ...
                            <div class="row">
                                @foreach($info->medias as $key => $row)
                                    <div class="col-sm-3" id="m_area{{ $key }}">
                                        <div class="card">
                                            <div class="card-body">
                                                <img src="{{ asset($row->url) }}" alt="" height="100" width="100">
                                            </div>
                                            <div class="card-footer">
                                                <button class="btn btn-danger col-12"
                                                        onclick="remove_image('{{ base64_encode($row->id) }}',{{ $key }} , {{ '"' . app()->getLocale() . '"'}})">{{ __('Remove') }}</button>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
