@extends('layouts.app')
@section('head')
    @include('layouts.partials.headersection',['title'=>trans('Create Product')])
@endsection
@section('content')

    <div class="row">
        <div class="col-lg-9">
            <form method="post" action="{{ route('seller.product.store') }}" id="product_create">
                @csrf

                <div class="card">
                    <div class="card-body">
                        @if ($errors->any())
                            <div class="alert alert-danger">
                                <ul>
                                    @foreach ($errors->all() as $error)
                                        <li>{{ $error }}</li>
                                    @endforeach
                                </ul>
                            </div>
                        @endif
                        <div class="row">
                            <div class="col-md-3">
                                <label for="price" class="mt-2">{{ __('Product Title') }} ({{__('Arabic')}})</label>
                            </div>
                            <div class="form-group col-md-9 col-12">

                                <input type="text" class="form-control"
                                       placeholder="{{ __('Product Title') }} ({{__('Arabic')}})" name="title_ar"
                                       required>

                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-3">
                                <label for="price" class="mt-2">{{ __('Product Title') }} ({{__('English')}})</label>
                            </div>
                            <div class="form-group col-md-9 col-12">

                                <input type="text" class="form-control"
                                       placeholder="{{ __('Product Title') }} ({{__('English')}})" name="title_en"
                                       required>

                            </div>

                            
                        </div>
                        <div class="row">
                            <div class="col-md-3">
                                <label for="price" class="mt-2">{{ __('Price') }}</label>
                            </div>
                            <div class="form-group col-md-9 col-12">

                                <input type="number" step="any" class="form-control" id="price"
                                       placeholder="{{ __('Price') }}" name="price" required="">

                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-3">
                                <label for="is_refundable" class="mt-2">{{ __('Allow Refund') }}</label>
                            </div>
                            <div class="form-group col-md-9 col-12">

                                <label>
                                    <input type="checkbox" id="is_refundable" name="is_refundable"
                                           class="custom-switch-input sm" value="1">
                                    <span class="custom-switch-indicator"></span>
                                </label>

                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-3">
                                <label for="has_offer" class="mt-2">{{ __('Has Offer') }}</label>
                            </div>
                            <div class="form-group col-md-9 col-12">

                                <label>
                                    <input type="checkbox" id="has_offer" name="has_offer"
                                           class="custom-switch-input sm" value="1">
                                    <span class="custom-switch-indicator"></span>
                                </label>

                            </div>
                        </div>
                        <div id="offer-details" style="display: none">


                            <div class="row">
                                <div class="col-md-3">
                                    <label for="special_price" class="mt-2">{{ __('Special Price') }}</label>
                                </div>
                                <div class="form-group col-md-9 col-12">
                                    <input type="number" step="any" class="form-control" id="special_price"
                                           placeholder=""
                                           name="special_price">
                                </div>
                            </div>

                            <div class="row">
                                <div class="col-md-3">
                                    <label for="special_price_type" class="mt-2">{{ __('Special Price Type') }}</label>
                                </div>
                                <div class="form-group col-md-9 col-12">
                                    <select name="price_type" id="special_price_type" class="form-control selectric">
                                        <option value="1">{{__('Fixed')}}</option>
                                        <option value="0">{{__('Percent')}}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-3">
                                    <label for="special_price_start"
                                           class="mt-2">{{ __('Special Price Start') }}</label>
                                </div>
                                <div class="form-group col-md-9 col-12">
                                    <input type="date" class="form-control" id="special_price_start" placeholder=""
                                           name="special_price_start">
                                </div>
                            </div>

                            <div class="row">
                                <div class="col-md-3">
                                    <label for="special_price_end" class="mt-2">{{ __('Special Price End') }}</label>
                                </div>
                                <div class="form-group col-md-9 col-12">
                                    <input type="date" class="form-control" id="special_price_end" placeholder=""
                                           name="special_price_end">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-3">
                                <label for="sku" class="mt-2">{{ __('SKU') }}</label>
                            </div>
                            <div class="form-group col-md-9 col-12">

                                <input type="text" class="form-control" id="sku" placeholder="#ABC-123" name="sku">

                            </div>
                        </div>


                        <div class="row">
                            <div class="col-md-3">
                                <label for="sku" class="mt-2">{{ __('Manage Stock') }}</label>
                            </div>
                            <div class="form-group col-md-9 col-12">

                                <label>
                                    <input type="checkbox" name="stock_manage" class="custom-switch-input sm" value="1">
                                    <span class="custom-switch-indicator"></span>
                                </label>

                            </div>
                        </div>

                        <div class="row">
                            <div class="col-md-3">
                                <label for="qty" class="mt-2">{{ __('Stock Quantity') }}</label>
                            </div>
                            <div class="form-group col-md-9 col-12">

                                <input type="number" class="form-control" id="qty"
                                       placeholder="{{ __('Stock Quantity') }}" name="stock_qty">

                            </div>
                        </div>


                    </div>
                </div>

        </div>
        <div class="col-lg-3">
            <div class="single-area">
                <div class="card">
                    <div class="card-body">

                        <div class="btn-publish">
                            <button type="submit" class="btn btn-primary col-12" id="submit_btn"><i
                                    class="fa fa-save"></i> {{ __('Save') }}</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    </form>

@endsection
@push('js')
    <Script>
        if ($("#has_offer").is(':checked')) {
            $('#offer-details').css("display", "block")
            $('#offer-details').val(1);
            $('#offer-details').prop('required', true);
            $('#special_price').prop('required', true);
            $('#special_price_start').prop('required', true);
            $('#special_price_end').prop('required', true);

        }
        $("#has_offer").on('change', function () {
            if ($("#has_offer").is(':checked')) {
                $('#offer-details').css("display", "block")
                $('#offer-details').val(1);
                $('#offer-details').prop('required', true);
                $('#special_price').prop('required', true);
                $('#special_price_start').prop('required', true);
                $('#special_price_end').prop('required', true);

            } else {
                $('#offer-details').css("display", "none")
                $('#offer-details').prop('required', false);
                $('#special_price').prop('required', false);
                $('#special_price_start').prop('required', false);
                $('#special_price_end').prop('required', false);
                $('#offer-details').val(0);
                $('#special_price').val(null);
                $('#special_price_start').val(null);
                $('#special_price_end').val(null);
            }
        });
    </Script>
    <script src="{{ asset('assets/seller/product/index.js') }}"></script>
@endpush
