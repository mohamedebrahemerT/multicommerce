                    ...
                    <div class="single-product-info">
                        <div class="single-product-meta">
                            ...
                            <ul class="single-page-slider round-icon">
                                ...
                                @if($previous)
                                    <li><a href="{{ url('/product/'.$previous->slug.'/'.$previous->id) }}"><i
                            ...
                            <div class="single-product-price">
                                @if($info->price->starting_date != null)
                                    ...
                                    <h3>
                                        ...
                                        <span id="amount">{{ amount_format($info->price->price) }}</span></h3>
                                @else
                        ...
                        <form method="post" class="cart-form" action="{{ url('/addtocart') }}">
                            ...
                            <div class="single-product-widget product-quantity">
                                ...
                                <ul>
                                    <li><input type="number" name="qty" id="qty"
                                               @if($info->stock->stock_manage == 1) @if($info->stock->stock_status == 0) disabled
                                               max="0" min="0" @else max="{{ $info->stock->stock_qty }}" min="1"
                                               value="1" @endif @else min="1" max="999" value="1" @endif></li>
                                    <li>
                                        <button type="submit" class="btn btn-outline submit_btn"
                                                @if($info->stock->stock_status == 0) disabled @endif>
                                            <i class="fas fa-shopping-basket"></i>
                                            <span
                                                class="submit_text"> @if($info->stock->stock_status == 0) {{ __('Out Of Stock') }} @else {{ __('Add to Cart') }} @endif</span>
                                        </button>
                                    </li>
                                    <li>
                                        <a href="javascript:void(0)" data-id="{{ $info->id }}" class="wishlist-icon"
                                           id="wishlist">
                                ...
                                <span
                                    class="available_qty @if($info->stock->stock_manage == 0 || $info->stock->stock_status == 0) none  @endif">{{ __('Available Quantity') }} : <span
                                        id="qty_area">@if($info->stock->stock_manage == 1) {{ $info->stock->stock_qty }} @endif</span></span>
                            ...
                            @foreach ($variations as $key => $item)
                                <div class="single-product-widget product-tags">
                                    ...
                                    <ul>
                                        @foreach ($item as $row)
                                            <li><label
                                                    class="attribute attr{{ $row->variation->id }}">{{ $row->variation->name }}
                                                    <input type="radio" class="variation" hidden
                                                           value="{{ $row->variation->id }}"
                                                           name="variation[{{ $row->attribute->id }}]"></label></li>
                                        @endforeach
                                    </ul>
                                </div>
                            @endforeach
                            ...
                            @foreach ($info->options as $key => $option)
                                ...
                                <div class="single-product-widget product-tags">
                                    <h5>{{ $option->name }} @if($option->is_required == 1) <span
                                            class="text-danger">*</span> @endif </h5>
                                    <ul>
                                ...
                                <ul>
                                    @foreach($info->categories as $row)
                                        <li><a class="text-dark"
                                               href="{{ url('/category/'.$row->slug.'/'.$row->id) }}">{{ $row->name }}</a>,
                                        </li>
                            ...
                            <div class="single-product-widget product-tags">
                                ...
                                <ul>
                                    @foreach($info->brands as $row)
                                        <li><a href="{{ url('/brand/'.$row->slug.'/'.$row->id) }}">{{ $row->name }}</a>
                                        </li>
                                        <input type="hidden" class="cat_id" value="{{ $row->id}}">
                                    @endforeach
                        ...
                        @if($info->is_refundable)
                            <div class="single-product-widget product-share mt-5">
                                <h5>{{ __('Allow Refund') }}</h5>
                                <img class="media-img-name" style="width: 30%"
                                     src="{{ asset('assets/img/refund.png') }}"/>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="col-lg-12">
                    <div class="details-menu">
                        <ul class="nav nav-tabs">
                            <li><a href="#descrip" class="nav-link active" data-toggle="tab">{{ ('Description') }}</a>
                            </li>
                            <li><a href="#review" class="nav-link" data-toggle="tab">{{ __('Review') }} <span
                                        id="review_count"></span></a></li>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="section-heading">
                        ...
                        <a href="{{ url('/shop') }}" class="btn btn-outline"><i
                                class="fas fa-eye"></i> {{ __('view all') }}</a>
                    </div>
                </div>
            </div>
