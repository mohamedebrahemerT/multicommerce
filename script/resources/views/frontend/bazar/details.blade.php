...
@section('content')
                                    ...
                                    @if(count($info->medias) > 0)
                                        @foreach($info->medias as $row)
                                            <div class="pd-o-img-wrap" data-src="{{ asset($row->url) }}">
                                                <img class="u-img-fluid" src="{{ asset($row->url) }}"
                                                     data-zoom-image="{{ asset($row->url) }}" alt="">
                                            </div>
                                        @endforeach
                                    @else(count($info->medias) == 0)
                                        <div class="pd-o-img-wrap" data-src="{{ asset('uploads/default.png') }}">
                                            <img class="u-img-fluid" src="{{ asset('uploads/default.png') }}"
                                                 data-zoom-image="{{ asset('uploads/default.png') }}" alt="">
                                        </div>
                                    @endif
                                </div>
                        ...
                        <div class="pd-detail">
                            ...
                            <div>
                                ...
                                @else
                                    <span class="pd-detail__price"
                                          id="amount">{{ amount_format($info->price->price) }}</span>
                                @endif
                            ...
                            <div class="u-s-m-b-15">
                                ...
                                <div class="pd-detail__rating gl-rating-style">
                                        ...
                                        <a data-click-scroll="#view-review"><span
                                                class="review_count"></span></a></span></div>
                            </div>
                            ...
                            <div class="u-s-m-b-15">
                                <span class="pd-detail__preview-desc">{{ content($content->excerpt ?? '') }}</span>
                            </div>
                            <div class="u-s-m-b-15">
                                <div class="pd-detail__inline">
                                        <span class="pd-detail__click-wrap"><i class="far fa-heart u-s-m-r-6 heart"></i>
                                            ...
                                            <a href="javascript:void(0)" data-id="{{ $info->id }}"
                                               id="wishlist">{{ __('Add to Wishlist') }}</a>
                            ...
                            <div class="u-s-m-b-15">
                                <ul class="pd-social-list">
                                    <li>
                                        ...
                                        <a class="s-fb--color-hover" target="_blank"
                                           href="https://www.facebook.com/sharer/sharer.php?u={{ url()->full() }}"><i
                                                class="fab fa-facebook-f"></i></a>
                                    </li>
                                    <li>

                                        <a class="s-tw--color-hover" target="_blank"
                                           href="https://twitter.com/intent/tweet?url={{ url()->full() }}"><i
                                                class="fab fa-twitter"></i></a>
                                    </li>
                                    <li>

                                        <a class="s-insta--color-hover" target="_blank"
                                           href="https://www.linkedin.com/sharing/share-offsite/?url={{ url()->full() }}"><i
                                                class="fab fa-linkedin-in"></i></a>
                                    </li>
                                    <li>

                                        <a class="s-wa--color-hover" target="_blank"
                                           href="https://wa.me/?text={{ url()->full() }}"><i
                                                class="fab fa-whatsapp"></i></a>
                                    </li>
                                    <li>
                                        <a class="s-gplus--color-hover" target="_blank"
                                ...
                                <form method="post" class="pd-detail__form cart-form" action="{{ url('/addtocart') }}">
                                        ...
                                        @foreach ($variations as $key => $item)
                                            ...
                                            <div class="pd-detail__color">
                                                ...
                                                @foreach ($item as $k => $row)
                                                    <div class="radio-box">
                                                        <input type="radio" @if($k == 0) checked
                                                               @endif id="attr{{ $row->variation->id }}"
                                                               value="{{ $row->variation->id }}"
                                                               name="variation[{{ $row->attribute->id }}]">
                                                        <div class="radio-box__state radio-box__state--primary">
                                                            <label
                                                                class="radio-box__label  attr{{ $row->variation->id }}"
                                                                for="attr{{ $row->variation->id }}">{{ $row->variation->name }}</label>&nbsp&nbsp
                                                        </div>
                                                    </div>
                                    ...
                                    <div class="u-s-m-b-15">
                                        @foreach ($info->options as $key => $option)
                                            <span
                                                class="pd-detail__label u-s-m-b-8">{{ $option->name }} @if($option->is_required == 1)
                                                    <span class="text-danger">*</span> @endif </span>
                                            <div class="pd-detail__size">
                                                @foreach ($option->childrenCategories as $row)
                                                    <div
                                                        class="@if($option->select_type == 1) size__checkbox @else size__radio @endif">
                                                        <input data-amount="{{ $row->amount }}"
                                                               data-amounttype="{{ $row->amount_type }}"
                                                               @if($option->select_type == 1) type="checkbox"
                                                               @else type="radio" @endif name="option[]"
                                                               value="{{ $row->id }}"
                                                               class="selectgroup-input option options @if($option->is_required == 1) req @endif"
                                                               id="option{{ $row->id }}">
                                    ...
                                    <div class="pd-detail-inline-2">
                                        <div class="u-s-m-b-15">
                                            ...
                                            <div class="input-counter">
                                                ...
                                                <input name="qty" id="qty"
                                                       ...
                                                       type="text"
                                                       @if($info->stock->stock_manage == 1) @if($info->stock->stock_status == 0) disabled
                                                       data-max="0" data-min="0"
                                                       @else data-max="{{ $info->stock->stock_qty }}" data-min="1"
                                                       value="1" @endif @else data-min="1" data-max="999"
                                                       value="1" @endif>
                                        ...
                                        <div class="u-s-m-b-15">
                                            <button class="btn btn--e-brand-b-2 submit_btn"
                                                    @if($info->stock->stock_status == 0) disabled
                                                    @endif type="submit">@if($info->stock->stock_status == 0)
                                                    <del>{{ __('Out Of Stock') }}</del> @else {{ __('Add to Cart') }} @endif
                                            </button>
                                        </div>
                                    </div>
                                </form>
                                    ...
                                    <ul class="pd-detail__policy-list d-flex">
                                        @foreach($info->categories as $row)
                                            <li><a class="text-dark"
                                                   href="{{ url('/category/'.$row->slug.'/'.$row->id) }}"><span>{{ $row->name }}</span></a>,
                                            </li>
                                ...
                                @if(count($info->brands) > 0)
                                    ...
                                    <ul class="pd-detail__policy-list">
                                        @foreach($info->brands as $row)
                                            <li><a class="text-dark"
                                                   href="{{ url('/brand/'.$row->slug.'/'.$row->id) }}"><span>{{ $row->name }}</span></a>,
                                            </li>
                                            <input type="hidden" class="cat_id" value="{{ $row->id}}">
                                        @endforeach
                                ...
                                <ul class="nav pd-tab__list">
                                    <li class="nav-item">
                                        <a class="nav-link active" data-toggle="tab"
                                           href="#pd-desc">{{ __('Description') }}</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" data-toggle="tab"
                                           href="#pd-tag">{{ __('Product Information') }}</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="view-review" data-toggle="tab" href="#pd-rev"><span
                                                class="review_count"></span></a>
                                    </li>
                                </ul>
                            </div>
                                    ...
                                    <div class="pd-tab__rev">
                                        <div class="u-s-m-b-30">
                                            <div class="pd-tab__rev-score">
                                                ...
                                                <span
                                                    class="gl-text">{{ __('Tell us what you think about this item') }}</span>
                                            </div>
                                        </div>
                                        <div class="u-s-m-b-30">
                                            <div class="rev-f1__group">
                                                <div class="u-s-m-b-15">
                                                    <h2><span
                                                            class="review_count"></span> {{ __('Review(s) for') }} {{ $info->title }}
                                                    </h2>
                                                </div>
                                        ...
                                        <div class="u-s-m-b-30">
                                            ...
                                            <form class="pd-tab__rev-f2 review-form" method="post"
                                                ...
                                                <div class="rev-f2__group">
                                                    <div class="u-s-m-b-15">
                                                        <label class="gl-label"
                                                               for="reviewer-text">{{ __('Your Review') }}
                                                            *</label><textarea
                                                            class="text-area text-area--primary-style"
                                                            id="reviewer-text" required name="comment"
                                                            maxlength="200"></textarea>
                                                    </div>
                                                    <div>

                                                        <p class="u-s-m-b-30">
                                                            <label class="gl-label" for="reviewer-name">{{ __('Name') }}
                                                                *</label>
                                                        ...
                                                        <p class="u-s-m-b-30">
                                                            <label class="gl-label"
                                                                   for="reviewer-email">{{ __('Email') }} *</label>
                                                            <input class="input-text input-text--primary-style"
                                                                   type="email" id="reviewer-email" name="email"
                                                ...
                                                <div>
                                                    @if(Auth::check())
                                                        <button class="btn btn--e-brand-shadow review_btn"
                                                                type="submit"><i
                                                                class="fas fa-paper-plane"></i> {{ __('Send Review') }}
                                                        </button>
                                                    @else
                                                        <a href="{{ url('/customer/login') }}"
                                                           class="btn btn--e-brand-shadow review_btn">
    ...
    <!--====== End - App Content ======-->
    <input type="hidden" id="max_qty" @if($info->stock->stock_manage == 1) value="{{ $info->stock->stock_qty }}"
           @else value="999" @endif>
