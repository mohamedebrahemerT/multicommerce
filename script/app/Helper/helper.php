...
function getUserOptionWithKey($key){
	$user_id = domain_info('user_id');
	$user_options = Useroption::where('user_id',$user_id)->where('key', $key)->first();
	if($user_options){
		return $user_options->value ;
	}else{
		return false ;
	}
}
...
function getUserId()
{
	if(Auth()->user() && Auth()->user()->role_id == 3 && Auth()->user()->is_admin == 0 && Auth()->user()->user_id != null){
...
function site_info()
{
       ...
       return $info = json_decode($site_info->value);
...
if (!function_exists('accountTranslation')) {
    function accountTranslation($key,$shop_number, $locale) {
		$data = [] ;
		if($shop_number == 4){
			$data['Products']['ar'] = 'الخدمات' ;
			$data['Products']['en'] = 'Services' ;

			$data['Product']['ar'] = 'الخدمة' ;
			$data['Product']['en'] = 'Service' ;

			$data['All Products']['ar'] = 'كل الخدمات' ;
			$data['All Products']['en'] = 'All Services' ;
		}else{
			$data['Products']['ar'] = 'المنتجات' ;
			$data['Products']['en'] = 'Products' ;

			$data['Product']['ar'] = 'المنتج' ;
			$data['Product']['en'] = 'Product' ;

			$data['All Products']['ar'] = 'كل المنتجات' ;
			$data['All Products']['en'] = 'All Products' ;
		}		
		if(isset($data[$key]) && isset($data[$key][$locale])){
...
function google_analytics_for_user()
{
	if (Auth::check()) {
		if (Auth::user()->role_id == 1) {
			...
			$info=Useroption::where('key','google-analytics')->where('user_id',getUserId())->first();
			$info=json_decode($info->value);
			$data['view_id']=$info->analytics_view_id;
	        $data['service_account_credentials_json']='uploads/'.getUserId().'/service-account-credentials.json';
	        return $data;
		}
	}
}
...
function user_limit()
{
	...
	$product_limit		=	$user->userlimit->product_limit ?? 0;
	$storage			=	$user->userlimit->storage ?? 0;
	$customer_limit		=	$user->userlimit->customer_limit ?? 0;
	$category_limit		=	$user->userlimit->category_limit ?? 0;
	$location_limit		=	$user->userlimit->location_limit ?? 0;
	$brand_limit		=	$user->userlimit->brand_limit ?? 0;
	$variation_limit	=	$user->userlimit->variation_limit ?? 0;
