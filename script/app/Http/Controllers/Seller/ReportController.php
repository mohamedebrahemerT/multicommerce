...
class ReportController extends Controller
{
     ...
     public function taxes(Request $request)
    {
        ...
        if ($request->start) {
            ...
            $total=Order::where('tax','>',0)->where('user_id',$user_id)->whereBetween('created_at',[$start,$end])->count();
            $completed=Order::where('tax','>',0)->where('user_id',$user_id)->whereBetween('created_at',[$start,$end])->where('status','completed')->count();
            $canceled=Order::where('tax','>',0)->where('user_id',$user_id)->whereBetween('created_at',[$start,$end])->where('status','canceled')->count();
            $proccess=Order::where('tax','>',0)->where('user_id',$user_id)->whereBetween('created_at',[$start,$end])->where([
                ['status','!=','completed'],
                ['status','!=','canceled'],
            ])->count();


            $amounts=Order::where('tax','>',0)->where([
                ['user_id',$user_id]
            ])->whereBetween('created_at',[$start,$end])->sum('total');
            $amount_cancel=Order::where('tax','>',0)->where([
                ['user_id',$user_id],
                ['status','canceled'],
                ['payment_status',0]
            ])->whereBetween('created_at',[$start,$end])->sum('total');
            $amount_proccess=Order::where('tax','>',0)->where([
                ['user_id',$user_id],
                ['status','!=','completed'],
                ['status','!=','canceled'],
                
            ])->whereBetween('created_at',[$start,$end])->sum('total');
            $amount_completed=Order::where('tax','>',0)->where([
                ['user_id',$user_id],
                ['status','completed'],
                ['payment_status',1]
            ])->whereBetween('created_at',[$start,$end])->sum('total');
            ...
            $orders=Order::where('tax','>',0)->where('user_id',$user_id)->whereBetween('created_at',[$start,$end])->with('customer')->withCount('order_items')->orderBy('id','DESC')->paginate(40);
        }
        else{
            $orders=Order::where('tax','>',0)->where([
                ['user_id',$user_id]
            ])->with('customer')->withCount('order_items')->orderBy('id','DESC')->paginate(40);
            $total=Order::where('tax','>',0)->where('user_id',$user_id)->count();
            $completed=Order::where('tax','>',0)->where('user_id',$user_id)->where('status','completed')->count();
            $canceled=Order::where('tax','>',0)->where('user_id',$user_id)->where('status','canceled')->count();
            $proccess=Order::where('tax','>',0)->where('user_id',$user_id)->where([
                ['status','!=','completed'],
                ['status','!=','canceled'],
            ])->count();
