...
//        dd(app()->getLocale());
	...
	public function staticData()
	{
        ...
        $earnings=Order::where('user_id',$user_id)->whereYear('created_at', '=',$year)->where('status','completed')->selectRaw('year(created_at) year, monthname(created_at) month, sum(total) total')
                ->groupBy('year', 'month')
                ->get();
        $posts=Term::where('user_id',getUserId())->where('type','!=','page')->count();
        $pages=Term::where('user_id',getUserId())->where('type','page')->count();
	...
	public function google_analytics($days)
	{
		...
		if (file_exists('uploads/'.getUserId().'/service-account-credentials.json')) {
