...
class Lphelper
{
	...
	public static function ConfigCategory($type,$select = ''){
		...
		if (Auth::user()->role_id==3) {
			$categories = Category::where('user_id',getUserId())->whereNull('p_id')->where('name','!=','Uncategorizied' )
//                ->select('id','name','p_id')
                ->where('type',$type)
...
//                ->select('id','name','p_id')
	...
	public static function ConfigCategoryMulti($type,$select = []){
		...
		if (Auth::user()->role_id==3) {
			$categories = Category::where('user_id',getUserId())
                ->whereNull('p_id')
	...
	public static function LPAdminCategory($type){
		if (Auth::user()->role_id==3) {
			$categories = Category::where('user_id',getUserId())->whereNull('p_id')->select('id','name','p_id')->where('type',$type)
			->with('childrenCategories')
			->get();
		}
		else{
